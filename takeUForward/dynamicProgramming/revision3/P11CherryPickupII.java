...
public class P11CherryPickupII {
    ...
    private int f(int i, int j1, int j2, int[][] matrix, int[][][] dp) {
        ...
        for ( int dj1=-1; dj1<=1; dj1++){
            for ( int dj2=-1; dj2<=1; dj2++){
                if ( j1== j2) max = Math.max(max, matrix[i][j1]+ f(i+1, j1+dj1, j2+dj2, matrix, dp));
                else max = Math.max(max, matrix[i][j1]+ matrix[i][j2] + f(i+1, j1+dj1, j2+dj2, matrix, dp));
            }
        }
